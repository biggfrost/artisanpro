...
import { LayoutDashboard, FileText, HardHat, Users, UserCircle2, MessageCircle, Bell } from 'lucide-react'
import { useUnreadMessages } from '../hooks/useUnreadMessages'
import { useNotifications } from '../contexts/NotificationsContext'

const NAV = [
  { to: '/manager/dashboard',  icon: LayoutDashboard, label: 'Accueil'   },
  { to: '/manager/devis',      icon: FileText,        label: 'Devis'     },
  { to: '/manager/chantiers',  icon: HardHat,         label: 'Chantiers' },
  { to: '/manager/clients',    icon: UserCircle2,     label: 'Clients'   },
  { to: '/manager/ouvriers',   icon: Users,           label: 'Équipe'    },
  { to: '/manager/messages',   icon: MessageCircle,   label: 'Messages',  badge: 'messages' },
]
          ...
          {NAV.map(({ to, icon: Icon, label, badge }) => {
            ...
            return (
              <NavLink
                ...
                to={to}
                className={({ isActive }) =>
                  `flex-1 flex flex-col items-center justify-center pt-3 pb-safe gap-0.5 relative transition-colors min-h-[60px] ${
              ...
              >
                {({ isActive }) => (
                  <>
                    ...
                    <span className="text-[10px] font-medium pb-1">{label}</span>
                  </>
                )}
              </NavLink>
            )
